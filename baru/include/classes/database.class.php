...
class database{
    ...
    */
	...
	var $con; // variable for tion id
	var $con_string; // variable for tion string
	...
	/**
	* Constructor of class - Initializes class and ts to the database
	...
	* @desc Constructor of class - Initializes class and ts to the database.
	*
	...
	*/
	function database($database_type,$host,$database,$user,$password,$port=false,$dsn=false){
		...
		// Setting database type and t to database
		if(in_array($database_type,$this->database_types)){
			$this->database_type=$database_type;
			
			$this->db_t=$this->database_type."_t";
	...
	/**
	* This function ts the database
	* @return boolean $is_ted Returns true if tion was successful otherwise false
	* @desc This function ts to the database which is set in the constructor
	*/
	function t(){
		// Selecting tion function and ting
		
		if($this->con==""){
			// INFORMIX
			if($this->database_type=="ifx"){
				$this->con=call_user_func($this->db_t,$this->database."@".$this->host,$this->user,$this->password);
			}else if($this->database_type=="mysql"){
				// With port
				if(!$this->port){
					$this->con=call_user_func($this->db_t,$this->host.":".$this->port,$this->user,$this->password);
				}
				// Without port
				else{
					$this->con=call_user_func($this->db_t,$this->host,$this->user,$this->password);
				}
				// mSQL
				...
				$this->con=call_user_func($this->db_t,$this->dsn,$this->user,$this->password);
				// Postgres SQL
			}else if($this->database_type=="pg"){
				// With port
				if(!$this->port){
					$this->con=call_user_func($this->db_t,"host=".$this->host." port=".$this->port." dbname=".$this->database." user=".$this->user." password=".$this->password);
				}
				// Without port
				else{
					$this->con=call_user_func($this->db_t,"host=".$this->host." dbname=".$this->database." user=".$this->user." password=".$this->password);
				}
				// Sybase
			...
			if(!$this->con){
				$this->halt("Wrong tion data! Can't establish tion to host.");
				return false;
			}else{
			...
			$this->halt("Already ted to database.");
			return false;
		}
	}
	
	/**
	* This function dists from the database
	* @desc This function dists from the database
	*/
	function dist(){
		if(@call_user_func($this->db_close,$this->con)){
			return true;
		}else{
			$this->halt("Not ted yet");
			return false;
		}
	}
	...
	function query($sql_statement){
		...
		if($this->database_type=="odbc"){
			...
			if(!$this->query_id=call_user_func($this->db_query,$this->con,$this->sql)){
				$this->halt("No database tion exists or invalid query");
			}else{
				if (!$this->query_id) {
